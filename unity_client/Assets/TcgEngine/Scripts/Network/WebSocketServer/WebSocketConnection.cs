...
namespace WebSocketServer {
    ...
    public struct WebSocketMessage {
        public WebSocketMessage(WebSocketConnection connection, byte[] data) {
    ...
    public struct WebSocketEvent {
        public WebSocketEvent(WebSocketConnection connection, WebSocketEventType type, byte[] data) {
            this.id = Guid.NewGuid().ToString();
            this.connection = connection;
            this.type = type;
            this.data = data;
    ...
    public class WebSocketConnection {
        ...
        {
            this.id = (ulong)Guid.NewGuid().ToString().GetHashCode();
            this.client = client;
        ...
        private void HandleConnection () {
            while (true) {
                ...
                if (dataframe.fin) {
                    if ((WebSocketOpCode)dataframe.opcode == WebSocketOpCode.Binary) {
                        // Let the server know of the message.
                        string data =  WebSocketProtocol.DecodeText(dataframe);
                        byte[] byteArray = Encoding.UTF8.GetBytes(data);
                        WebSocketEvent wsEvent = new WebSocketEvent(this, WebSocketEventType.Message, byteArray);
                        server.events.Enqueue(wsEvent);
                    }else if ((WebSocketOpCode)dataframe.opcode == WebSocketOpCode.Text) {
                        // Let the server know of the message.
                        string data =  WebSocketProtocol.DecodeText(dataframe);
                        byte[] byteArray = Encoding.UTF8.GetBytes(data);
                        WebSocketEvent wsEvent = new WebSocketEvent(this, WebSocketEventType.Message, byteArray);
