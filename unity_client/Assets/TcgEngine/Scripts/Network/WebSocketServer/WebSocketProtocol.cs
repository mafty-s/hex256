...
namespace WebSocketServer {
    ...
    class WebSocketProtocol {
             ...
             public static byte[] EncodeText(string text)
             {
                 ...
                 {
                     frame = new byte[rawData.Length + 2];
                     frame[0] = 0b10000001; // Fin + Text opcode
                     frame[1] = (byte)rawData.Length;
                     Array.Copy(rawData, 0, frame, 2, rawData.Length);
                 }
                 else if (rawData.Length >= 126 && rawData.Length <= ushort.MaxValue)
                 {
                     frame = new byte[rawData.Length + 4];
                     frame[0] = 0b10000001; // Fin + Text opcode
                     frame[1] = 126;
                     frame[2] = (byte)((rawData.Length >> 8) & 255);
                     frame[3] = (byte)(rawData.Length & 255);
                     Array.Copy(rawData, 0, frame, 4, rawData.Length);
                 }
                 else
                 {
                     frame = new byte[rawData.Length + 10];
                     frame[0] = 0b10000001; // Fin + Text opcode
                     frame[1] = 127;
                     frame[2] = (byte)((rawData.Length >> 56) & 255);
                     frame[3] = (byte)((rawData.Length >> 48) & 255);
                     frame[4] = (byte)((rawData.Length >> 40) & 255);
                     frame[5] = (byte)((rawData.Length >> 32) & 255);
                     frame[6] = (byte)((rawData.Length >> 24) & 255);
                     frame[7] = (byte)((rawData.Length >> 16) & 255);
                     frame[8] = (byte)((rawData.Length >> 8) & 255);
                     frame[9] = (byte)(rawData.Length & 255);
                     Array.Copy(rawData, 0, frame, 10, rawData.Length);
                 }
