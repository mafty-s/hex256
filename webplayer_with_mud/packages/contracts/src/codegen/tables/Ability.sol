...
library Ability {
  ...
  function getValueSchema() internal pure returns (Schema) {
    ...
    _valueSchema[8] = SchemaType.UINT8_ARRAY;
  ...
  function set(
    ...
    PackedCounter _encodedLengths = encodeLengths(id, effects, status);
    bytes memory _dynamicData = encodeDynamic(id, effects, status);
  ...
  function set(bytes32 key, AbilityData memory _table) internal {
    ...
    PackedCounter _encodedLengths = encodeLengths(_table.id, _table.effects, _table.status);
    bytes memory _dynamicData = encodeDynamic(_table.id, _table.effects, _table.status);
  ...
  function decodeDynamic(
    ...
    bytes memory _blob
  ) internal pure returns (string memory id, bytes4[] memory effects, uint8[] memory status) {
    uint256 _start;
    uint256 _end;
    unchecked {
  ...
  function decode(
    ...
    (_table.id, _table.effects, _table.status) = decodeDynamic(_encodedLengths, _dynamicData);
  }
  ...
  function encodeLengths(
    ...
    unchecked {
      _encodedLengths = PackedCounterLib.pack(bytes(id).length, effects.length * 4, status.length * 1);
    }
  }
  ...
  function encodeDynamic(
    ...
    return abi.encodePacked(bytes((id)), EncodeArray.encode((effects)), EncodeArray.encode((status)));
  }
